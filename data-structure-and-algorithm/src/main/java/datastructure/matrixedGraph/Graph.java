...
public class Graph {
    ...
    // 比如path[w] = v，表示从起点到顶点w需要先经过w的父顶点 v
    ...
    // 广度优先搜索breadthFirstSearch，类似于树的层次遍历
    public void BFS() {
        System.out.print(vertexList[0].value + " "); //访问第一个顶点
        ...
        while (!queue.isEmpty()) {
            //寻找当前顶点没有访问的邻接点
            int v = getUnvisitedVertex((int) queue.peek());
            if (v != -1) {
                System.out.print(vertexList[v].value + " ");
                vertexList[v].visited = true;
                queue.add(v);
    ...
    /**
     * @Description: Dijkstra解决单源最短路径问题
    ...
    public void dijkstra(int s) {
        ...
        for (int i = 0; i < vertexList.length; i++) {
            ...
            for (int j = 0; j < vertexList.length; j++) {
                //如果还没有访问过，并且比当前值要小
                if (!vertexList[j].visited && distance[j] < min) {
                    vertexList[j].visited = true; //设置为已访问
                    min = distance[j];        //更新最小值
                    k = j;    //记录最小路径的顶点序号
                }
            }
            // 已知"顶点k的最短路径"之后，更新"未获取最短路径的顶点的最短路径和前驱顶点"。
            for (int j = 0; j < vertexList.length; j++) {
                ...
                if (!vertexList[j].visited && (tmp < distance[j])) {
    ...
    public void printDijkstra(int s) {
        ...
        for (int i = 0; i < vertexList.length; i++) {
            ...
            while (!st.isEmpty()) {
                System.out.printf("%c-->", vertexList[(int) st.pop()].value);
            }
            System.out.println();
        }
    }
     ...
     * 直白解释：https://www.cnblogs.com/wangyuliang/p/9216365.html
     * @return: void
     * @Author: ZHQ
     * @Date: 2019/8/10
     **/
    ...
    /**
     * @Description: Prim算法解决最小生成树问题，以顶点为思考对象
     * @Param: [start]
     * @return: void
     * @Author: ZHQ
     * @Date: 2019/8/10
    ...
    public void prim(int start) {
        //记录最小生成树的顶点序号
        int[] prims = new int[vertexList.length];
        //初始化
        for (int i = 0; i < vertexList.length; i++) {
            //到起点的权值
            distance[i] = mGraph[start][i];
            prims[i] = -1;
        }
        //自己到自己的距离为0
        distance[start] = 0;
    ...
    /**
     * @Description: 克鲁斯卡尔算法 Kruskal算法：求最小生成树  prim算法适合稠密图，kruskal算法适合简单图。
     *              涉及到并查集的合并
     * @return: void
     * @Author: ZHQ
     * @Date: 2019/8/10
    ...
    public void kruskal() {
        ...
        //记录结果的数组，边的顺序
        char[] result = new char[2 * vertexList.length - 2];
